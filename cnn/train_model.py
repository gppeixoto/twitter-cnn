...
model_path = '../models/03b0a0_Thu_Nov_24_09:05:40_2016.model' #emb_dim=200
cnn = convnet.SingleLayerCNN(
    SEQUENCE_LENGTH, 30, [4, 5, 6, 7], FEATURE_MAPS, model_path
    )

split = .7
embedding_reader = TweetCorpusReader('../data/', w2i=cnn.word2index())
...
pos_mask = (y==1).reshape(indices.shape[0],)
...
cnn.model.fit(x_train, y_train, nb_epoch=3)
cnn_pred = cnn.model.predict(x_test)
...
text_reader = TweetCorpusReader('../data/', text_only=False, debug=True)
